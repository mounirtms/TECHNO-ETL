...
export const useGridPerformance = (options = {}) => {
  ...
  // Memory monitoring
  const estimateMemoryUsage = useCallback((dataSet as any) => {
    try {
      return JSON.stringify(dataSet as any).length * 2; // Rough estimate in bytes
    } catch {
  ...
  const loadPage = useCallback(async (pageIndex as any) => {
    if (!enableLazyLoading || loadingRef.current || loadedPages.has(pageIndex as any)) {
      return;
    }
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (!isNaN(pageIndex as any)) {
              loadPage(pageIndex as any);
            }
          }
        });
      },
  ...
  const handleScroll = useCallback((event as any) => {
  ...
  // Scroll to item
  const scrollToItem = useCallback((index as any) => {
