...
export const useDashboardParams = () => {
  ...
  const setView = (view: any) => updateParams({ view });
  const setFilter = (filter: any) => updateParams({ filter });
  const setStatus = (status: any) => updateParams({ status });
  const setSortBy = (sortBy: any) => updateParams({ sortBy });
  const setPeriod = (period: any) => updateParams({ period });
  const setCategory = (category: any) => updateParams({ category });
  const setPriority = (priority: any) => updateParams({ priority });
  const setAlert = (alert: any) => updateParams({ alert: alert.toString() });
...
export const useGridParams = () => {
  ...
  const setPage = (page as any) => updateParams({ page: page.toString() });
  const setPageSize = (pageSize: any) => updateParams({ pageSize: pageSize.toString() });
  const setSearchQuery = (search: any) => updateParams({ search });
  const setSelectedIds = (ids: any) => updateParams({ selected: JSON.stringify(ids) });
