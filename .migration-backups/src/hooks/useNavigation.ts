...
export const useNavigation = () => {
  ...
  const isRouteActive = useCallback((path as any) => {
    return location.pathname === path;
  }, [location.pathname]);

  // Check if route is accessible
  const isRouteAccessible = useCallback((path as any) => {
    const metadata = ROUTE_METADATA[path];
    return metadata && metadata.requiresAuth !== false;
  }, []);

  // Get route metadata
  const getRouteMetadata = useCallback((path as any) => {
 ...
 */
function generateBreadcrumbs(path as any) {
...
export const useMenuState = () => {
  ...
  const setActiveSubmenu = useCallback((submenu as any) => {
    setMenuState(prev => ({ ...prev, activeSubmenu: submenu }));
  }, []);

  const setHoveredItem = useCallback((item as any) => {
...
export const useBreadcrumbs = () => {
  ...
  const navigateToBreadcrumb = useCallback((breadcrumb as any) => {
