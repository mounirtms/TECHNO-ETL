...
const OptimizedDashboardWidgets = ({ 
  ...
  const fetchWidgetData = useCallback(async (widgetId as any) => {
  ...
  const handleRefresh = useCallback((widgetId as any) => {
    fetchWidgetData(widgetId as any);
  }, [fetchWidgetData]);

  // Handle widget visibility toggle
  const handleToggleVisibility = useCallback((widgetId as any) => {
    setWidgets(prev => ({
      ...prev,
      [widgetId]: {
        ...prev[widgetId],
  ...
  const handleSettings = useCallback((widgetId as any) => {
