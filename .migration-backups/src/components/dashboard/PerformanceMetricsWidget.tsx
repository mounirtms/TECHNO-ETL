...
const PerformanceMetricsWidget = () => {
  ...
  const MetricCard = ({ title, value, unit, status, trend, icon, isPercentage = false }) => (
    ...
    >
      <CardContent sx={{ p: density === 'compact' ? 1.5 : 2 } as any}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 } as any}>
          <Avatar sx={{ bgcolor: `${getStatusColor(status as any)}.light`, width: 32, height: 32 }}>
            {icon}
          </Avatar>
          {trend && getTrendIcon(trend)}
        </Box>
        
        <Typography variant="h6" fontWeight={600} color={`${getStatusColor(status as any)}.main`}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 } as any}>
          {title}
        </Typography>
        ...
        <Chip 
          ...
          sx={{ textTransform: 'capitalize', fontSize: '0.7rem', height: 20 } as any}
        />
      </CardContent>
    </Card>
  );
  ...
  const ProgressMetric = ({ title, value, max, status, icon }) => (
    <Box sx={{ mb: 2 } as any}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 } as any}>
        ...
        <Typography variant="body2" color={`${getStatusColor(status as any)}.main`} fontWeight={600}>
          {value}{max ? `/${max}` : '%'}
        </Typography>
      </Box>
      <LinearProgress
  ...
  return (
    <Card sx={{ 
      ...
      <CardContent sx={{ 
        ...
        <Grid {...{container: true}} spacing={1} sx={{ mb: 3 } as any}>
          <Grid size={6}>
            <MetricCard
              title="CPU Usage"
        ...
        <Box sx={{ 
          ...
          <Typography variant="caption" color="text.secondary" sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 1 } as any}>
