  ...
  return (
    ...
    >
      ...
      <CardHeader
        avatar={
          <Badge badgeContent={activities.filter(a => a.priority === 'high').length} color="error">
            <Avatar sx={{ bgcolor: 'info.main' } as any}>
              <ActivityIcon />
            </Avatar>
          </Badge>
        }
      ...
      <CardContent sx={{ 
        ...
        {/* Filter Chips */}
        <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' } as any}>
        ...
        {/* Activity List */}
        <List dense sx={{ maxHeight: 350, overflow: 'auto' } as any}>
          {filteredActivities.slice(0, 8).map((activity, index) => (
            <React.Fragment key={activity.id}>
              ...
              {index < filteredActivities.slice(0, 8).length - 1 && (
                <Divider variant="inset" component="li" sx={{ ml: 6 } as any} />
              )}
            </React.Fragment>
          ))}
        </List>
        ...
        {filteredActivities.length > 8 && (
          <Box sx={{ textAlign: 'center', mt: 2 } as any}>
