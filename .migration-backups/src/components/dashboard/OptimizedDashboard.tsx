    ...
    return performance.trackExpensiveCalculation(
      ...
      (data) => ({
        salesData: processGridRows(data??.sales || [], {
          revenue: (value as any) => parseFloat(value as any),
          growth: (value) => parseFloat(value as any)
        }),
        inventoryData: processGridRows(data??.inventory || [], {
          stock: (value as any) => parseInt(value as any),
          price: (value) => parseFloat(value as any)
        }),
        customerData: processGridRows(data??.customers || [], {
          orders: (value as any) => parseInt(value as any),
          value: (value) => parseFloat(value as any)
        })
      }),
      dashboardData
    );
  ...
  if (loading && !dashboardData) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" height="100vh">
        <CircularProgress />
        <Typography variant="body1" sx={{ ml: 2 } as any}>
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Box sx={{ p: 3 } as any}>
        ...
        {/* Date Range Selector */}
        <Paper sx={{ p: 2, mb: 3 } as any}>
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' } as any}>
            <DatePicker
              label={t('dashboard.startDate') || 'Start Date'}
              value={dateRange.start}
              onChange={(e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (newValue) => setDateRange(prev => ({ ...prev, start: newValue }))}
              renderInput={(params) => <TextField {...params} />}
            />
            <DatePicker
              label={t('dashboard.endDate') || 'End Date'}
              value={dateRange.end}
              onChange={(e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (newValue) => setDateRange(prev => ({ ...prev, end: newValue }))}
              renderInput={(params) => <TextField {...params} />}
            />
          </Box>
        </Paper>
        ...
        <Grid {...{container: true}} spacing={3}>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Paper sx={{ p: 2, height: '100%' } as any}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 } as any}>
                <Typography variant="h6">
