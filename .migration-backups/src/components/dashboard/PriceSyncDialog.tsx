 ...
 */
const PriceSyncDialog: React.FC<any> = ({ open, onClose, priceData = [], syncStatus: propSyncStatus, syncResults: propSyncResults, progress: propProgress, setSyncStatus: propSetSyncStatus, setSyncResults: propSetSyncResults, setProgress: propSetProgress, onSync }) => {
  ...
  const getStatusIcon = (status as any) => {
    switch (status) {
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ p: 3 } as any}>
        {/* Sync Overview */}
        <Grid {...{container: true}} spacing={3} sx={{ mb: 3 } as any}>
          <Grid {...{item: true}} xs={12} md={4}>
            <Card variant="outlined">
              <CardContent sx={{ textAlign: 'center' } as any}>
          ...
          <Box>
            ...
            <Collapse in={internalShowDetails}>
              ...
              {syncResults.results.length > 20 && (
                <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' } as any}>
      ...
      <DialogActions sx={{ p: 3, gap: 1 } as any}>
