 ...
 */
const EnhancedStatsCards: React.FC<any> = ({ 
  stats, 
  settings, 
  loading = false, 
  onNavigate,
  ...
  const getProgressColor = (card: any) => {
    if (card.isAlert) {
  ...
  const getProgressValue = (card as any) => {
    if (card.isAlert) {
      ...
      <Grid {...{container: true}} spacing={3}>
        {Array.from({ length: 8 }).map((_, index) => (
          <Grid size={{ xs: 12, md: 6, lg: 4, xl: 3 }} key={index}>
            <Card sx={{ height: 200, borderRadius: 3 } as any}>
              <CardContent sx={{ p: 3 } as any}>
                <Skeleton variant="circular" width={48} height={48} />
                <Skeleton variant="text" width="60%" sx={{ mt: 2 } as any} />
                <Skeleton variant="text" width="40%" />
                <Skeleton variant="rectangular" height={6} sx={{ mt: 2 } as any} />
              </CardContent>
            </Card>
          </Grid>
        ))}
    ...
    <Grid {...{container: true}} spacing={3}>
      {visibleCards.map((card, index) => (
        ...
        >
          ...
          >
            <CardContent sx={{ p: settings?.general?.compactMode ? 2 : 3, height: '100%' } as any}>
              {/* Header */}
              <Box sx={{ 
                display: 'flex', 
              ...
              <Box sx={{ mb: 2 } as any}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 } as any}>
                  <Chip
                    label={card.change}
                    size="small"
