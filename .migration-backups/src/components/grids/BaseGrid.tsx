...
const BaseGrid = forwardRef(({
  ...
  // Grid configuration
  getRowId = (row as any) => row.id || row.entity_id || row.sku,
  ...
  const processedData = useMemo(() => {
    ...
    return data.map((row, index) => ({
      ...row,
      _gridIndex: index,
      _gridId: getRowId(row as any) || `row-${index}`
    }));
  ...
  const handleSelectionChange = useCallback((newSelection as any) => {
    ...
    onSelectionModelChange?.(newSelection as any);
  }, [onSelectionModelChange]);

  const handleError = useCallback((error as any) => {
