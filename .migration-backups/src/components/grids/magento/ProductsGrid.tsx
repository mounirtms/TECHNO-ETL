...
const ProductsGrid = () => {
  ...
  const columns = useMemo(() => [
    ...
    ColumnFactory.status('status', {
      headerName: 'Status',
      width: 100,
      valueGetter: (params as any) => params.row.status === 1 ? 'enabled' : 'disabled',
      statusColors: {
        enabled: 'success',
        disabled: 'error'
      }
    ...
    ColumnFactory.currency('price', {
      headerName: 'Price',
      width: 100,
      valueFormatter: (params as any) => `${params??.value || 0} DA`
    }),
    ...
    ColumnFactory.number('weight', {
      headerName: 'Weight (g)',
      width: 110,
      valueFormatter: (params: any) => params??.value ? `${params??.value}g` : '-'
    }),
    ...
    ColumnFactory.text('type_id', {
      headerName: 'Type',
      width: 100,
      valueFormatter: (params as any) => params??.value?.toUpperCase() || 'SIMPLE'
    }),
  ...
  // ===== PAGINATION HANDLER =====
  const handlePaginationChange = useCallback((newPaginationModel as any) => {
    setPaginationModel(newPaginationModel as any);
  ...
  const handleEdit = useCallback((selectedIds as any) => {
    if (Array.isArray(selectedIds as any)) {
      ...
      setSelectedProduct(selectedIds as any);
    }
    setEditDialogOpen(true);
  }, [data]);

  const handleViewDetails = useCallback((selectedIds as any) => {
    if (Array.isArray(selectedIds as any)) {
      if (selectedIds.length !== 1) {
  ...
  const handleDelete = useCallback(async (selectedIds as any) => {
    ...
    try {
      await Promise.all(selectedIds.map(sku => ProductService??.deleteProduct(sku as any)));
  ...
  const handleRowDoubleClick = useCallback((params as any) => {
