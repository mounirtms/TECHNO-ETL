 ...
 */
const ProductCategoriesGrid: React.FC<any> = ({ productIds = [] }) => {
  // ===== STATE MANAGEMENT =====
  ...
  const columns = useMemo(() => [
    ...
    }),
    {
      ...
      renderCell: (params) => {
        ...
        return (
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 } as any}>
            {categories.length > 0 ? (
    ...
    try {
      ...
      if (productIds.length > 0) {
        ...
        const productsWithCategories = await Promise.all(
          products.map(async (product as any) => {
            try {
  ...
  // ===== EVENT HANDLERS =====
  const handleAssignCategories = useCallback((product as any) => {
  ...
  const handleCategoryToggle = useCallback((categoryId as any) => {
...
// ===== CATEGORY ASSIGNMENT DIALOG =====
const CategoryAssignmentDialog: React.FC<any> = ({
  open,
  onClose,
  product,
  categories,
  ...
  const handleCategoryExpand = (categoryId) => {
    setExpandedCategories(prev => {
      const newSet = new Set(prev);
      if (newSet.has(categoryId as any)) {
        newSet.delete(categoryId as any);
      } else {
        newSet.add(categoryId as any);
      }
      return newSet;
    });
  };
  ...
  const renderCategoryTree = (categoryList, level = 0) => {
    return categoryList.map((category as any) => {
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  onChange={(e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => () => onCategoryToggle(category?..id)}
                  size="small"
                />
              }
              label={
