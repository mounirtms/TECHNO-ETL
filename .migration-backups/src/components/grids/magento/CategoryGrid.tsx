 ...
 */
const CategoryGrid: React.FC<any> = ({ productId }) => {
    // State management
    ...
    const handleRowExpand = useCallback((id as any) => {
            ...
            return (
                <Box sx={{
                    ...
                    {hasChildren ? (
                        ...
                        <Box sx={{ width: 20, mr: 1 } as any} /> // Spacer for alignment
                    )}
    ...
    const handleRefresh = useCallback(async (params = {}) => {
        try {
            ...
            const formattedCategories = visibleCategories.map(category =>
                categoryService.formatCategoryForGrid(category as any)
            );
    ...
    const updateStats = useCallback((categories as any) => {
    ...
    return (
        <Box>
            ...
            contextMenuActions={{
                ...
                delete: {
                    enabled: (rowData: any) => rowData??.level > 1, // Don't allow deleting root categories
                    onClick: (rowData: any) => {
            ...
            floatingActions={{
                ...
                edit: {
                    enabled: (selectedRows: any) => selectedRows.length === 1,
                    priority: 2
                },
                export: {
                    enabled: true,
