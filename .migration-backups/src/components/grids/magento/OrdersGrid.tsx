...
const OrdersGrid = () => {
  ...
  // ===== 3. EVENT HANDLERS =====
  const handleView = useCallback((records as any) => {
    if (records.length === 1) {
  ...
  const handleEdit = useCallback((records as any) => {
    if (records.length === 1) {
      const order = data.find(o => o?.entity_id === records[0]);
      setSelectedOrder(order);
  ...
  const handleCancel = useCallback(async (records as any) => {
    if (records.length === 0) {
  ...
  const handleFilterChange = useCallback((newFilter as any) => {
