 ...
 */
const ProductManagementGrid: React.FC<any> = ({ initialProductIds = [] }) => {
  // ===== STATE MANAGEMENT =====
  ...
  const closeFloatingWindow = useCallback((windowType as any) => {
    setFloatingWindows(prev => ({
      ...prev,
  ...
  const handleManualRefresh = useCallback(async (type as any) => {
  ...
  const fetchProducts = useCallback(async (params = {}) => {
    try {
      ...
      if (selectedProducts.length > 0) {
        ...
        // Fetch all products
        const response = await magentoApi.getProducts(params as any);
        productsData = response?.items || [];

        // Enhance with additional attributes
        productsData = await Promise.all(
          productsData.map(async (product as any) => {
            try {
  ...
  const handleViewProduct = useCallback((product as any) => {
    console.log('👁️ Viewing product:', product);
    setSelectedProduct(product as any);
    setProductDetailOpen(true);
  }, []);

  const handleEditProduct = useCallback((product as any) => {
    console.log('✏️ Editing product:', product);
    toast.info(`Editing product: ${product.name}`);
  }, []);

  const handleManageCategories = useCallback((product as any) => {
          ...
          <UnifiedGrid
            {...getStandardGridProps('products', {
              ...
              onSelectionModelChange: (newSelection) => {
                setSelectedProducts(newSelection as any);
              },
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' } as any}>
            ...
            <Grid {...{container: true}} spacing={2}>
              ...
              <Grid size={{ xs: 12, sm: 6, md: 2 }}>
                <FormControl fullWidth size="small">
                  ...
                  <Select
                    value={filters.type}
                    onChange={(e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => handleFilterChange('type', e.target??.value)}
                    label="Type"
                  >
              ...
              <Grid size={{ xs: 12, sm: 6, md: 2 }}>
                <TextField
                  ...
                  onChange={(e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => handlePriceRangeChange('min', e.target??.value)}
                />
              </Grid>
              <Grid size={{ xs: 12, sm: 6, md: 2 }}>
                <TextField
                  ...
                  onChange={(e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => handlePriceRangeChange('max', e.target??.value)}
                />
              </Grid>

              {/* Active Filters Display */}
              <Grid size={12}>
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mt: 1 } as any}>
                  {filters.brand && (
                    <Chip
      ...
      {/* Tabs */}
      <Paper sx={{ mb: 2 } as any}>
        <Tabs
          value={currentTab}
          onChange={(e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => handleTabChange}
          variant="fullWidth"
      ...
      {/* Tab Content */}
      <Box sx={{ flexGrow: 1, overflow: 'hidden' } as any}>
        {renderTabContent()}
      </Box>
      ...
      <Box sx={{ position: 'fixed', bottom: 24, right: 24, zIndex: 1000 } as any}>
        <Stack spacing={2}>
          {/* Quick Actions FAB */}
      ...
      >
        <Paper sx={{ p: 2, minWidth: 250 } as any}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 } as any}>
      ...
      <Popover
        ...
        transformOrigin={{ vertical: 'center', horizontal: 'right' }}
      >
        <Paper sx={{ p: 2, minWidth: 280 } as any}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 } as any}>
...
// ===== PRODUCT DETAIL DIALOG =====
const ProductDetailDialog: React.FC<any> = ({ open, onClose, product }) => {
