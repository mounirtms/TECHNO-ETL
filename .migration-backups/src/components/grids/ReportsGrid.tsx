...
const getChartIcon = (chartType as any) => {
  switch(chartType as any) {
...
const ReportsGrid: React.FC<any> = ({ data, onDataChange, onBadgeUpdate }) => {
    ...
    <Box sx={{ p: 3 } as any}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(380px, 1fr))', gap: 2 } as any}>
        {filteredReports.map((report) => (
          <Card key={report.id} sx={{ height: 'fit-content' } as any}>
            <CardContent>
              <Stack spacing={2}>
                {/* Report Header */}
                ...
                {report.status === 'generating' && (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 } as any}>
                    <Box sx={{ 
                      width: 16, 
                      height: 16, 
      ...
      {filteredReports.length === 0 && (
        <Box sx={{ textAlign: 'center', py: 4 } as any}>
          <ReportIcon sx={{ fontSize: 64, color: 'text.secondary', mb: 2 } as any} />
