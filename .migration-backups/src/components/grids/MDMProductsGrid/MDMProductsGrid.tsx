...
const MDMProductsGrid = () => {
  ...
  const SyncProgressToast = useCallback(({ current, total }) => (
    <Box sx={{ width: '100%', p: 1 } as any}>
      <Typography variant="caption" sx={{ display: 'block', mb: 1 } as any}>
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      width: 100,
      type: 'number',
      renderCell: (params as any) => params?.value?.toString() || '0'
    },
    {
      field: 'VenteMoyen',
  ...
  const contextMenuActions = useMDMContextMenuActions({
    ...
    onView: (rowData as any) => console.log('View details:', rowData),
  ...
  const getRowClassName = useCallback((params as any) => params.row?..changed ? 'row-changed' : '', []);
  ...
  return (
    <>
      <Box sx={{ flexShrink: 0, mb: 0.5 } as any}>
        <MDMFilterPanel
      ...
      <UnifiedGrid
        {...getStandardGridProps('mdm', {
          ...
          loading: loading,
          onRefresh: handleManualRefresh,
          onRowDoubleClick: (params as any) => {
        ...
        onSync={onSyncHandler}
        onSelectionChange={(e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => setSelectedBaseGridRows}
