...
const RoadmapGrid = () => {
  ...
  const toggleSection = (status) => {
    setExpandedSections(prev => {
      ...
      if (newSet.has(status as any)) {
        newSet.delete(status as any);
      } else {
        newSet.add(status as any);
      }
      return newSet;
    });
  };
        ...
        {progressStats && (
          <Grid {...{container: true}} spacing={3}>
            <Grid {...{item: true}} xs={12} md={3}>
              <Card sx={{ textAlign: 'center', p: 2 } as any}>
              ...
              {Object.entries(STATUS_CONFIG).map(([status, config], index) => {
                ...
                return (
                  <TimelineItem key={status}>
                    ...
                    <TimelineContent>
                      <Accordion 
                        expanded={expandedSections.has(status as any)}
                        onChange={(e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => () => toggleSection(status as any)}
                        sx={{ mb: 2 } as any}
                      >
            ...
            {roadmapData?..byCategory && (
              <Stack spacing={2}>
                {Object.entries(roadmapData?..byCategory).map(([category, features]) => {
                  ...
                  return (
                    <Card key={category} variant="outlined" sx={{ p: 2 } as any}>
            ...
            {roadmapData?..recentActivity && roadmapData?..recentActivity.length > 0 ? (
              <Stack spacing={2}>
                {roadmapData?..recentActivity.slice(0, 5).map((activity, index) => (
                  <Box key={index} sx={{ pb: 1, borderBottom: index < 4 ? 1 : 0, borderColor: 'divider' } as any}>
