...
const getStockStatus = (stock as any) => {
  if (stock === 0) return 'out_of_stock';
  if (stock <= 10) return 'low_stock';
  return 'active';
};

const ProductsGrid: React.FC<any> = ({ data, onDataChange, onBadgeUpdate }) => {
  ...
  const renderStars = (rating) => {
    const stars = [];
    const fullStars = Math.floor(rating as any);
    const hasHalfStar = rating % 1 !== 0;

    for (let i = 0; i < fullStars; i++) {
      stars.push(<StarIcon key={i} sx={{ fontSize: 16, color: 'gold' } as any} />);
    }

    if (hasHalfStar) {
      stars.push(<StarIcon key="half" sx={{ fontSize: 16, color: 'gold', opacity: 0.5 } as any} />);
    }
  ...
  return (
    <Box sx={{ p: 3 } as any}>
      ...
      {/* Products Grid */}
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 2 } as any}>
        {filteredProducts.map((product) => (
          <Card key={product.id} sx={{ height: 'fit-content' } as any}>
            {/* Product Image */}
            <CardMedia
              sx={{
                height: 200,
            ...
            >
              {product.image ? (
                ...
                <ProductIcon sx={{ fontSize: 64, color: 'grey.400' } as any} />
              )}
            </CardMedia>
      ...
      {filteredProducts.length === 0 && (
        <Box sx={{ textAlign: 'center', py: 4 } as any}>
          <ProductIcon sx={{ fontSize: 64, color: 'text.secondary', mb: 2 } as any} />
