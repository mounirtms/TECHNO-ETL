...
const CustomersGrid = ({
  ...
  const handleEditCustomer = (customerId: any) => {
    ...
    <Box sx={{ p: 3 } as any}>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ mb: 3 } as any}>
        <TextField
          ...
          value={searchQuery}
          onChange={(e: any) => (e: any) => (e: any) => setSearchQuery(e.target.value)}
          InputProps={{
            startAdornment: (
        ...
        <FormControl sx={{ minWidth: 120 } as any}>
          ...
          <Select
            value={statusFilter}
            label="Status"
            onChange={(e: any) => (e: any) => (e: any) => setStatusFilter(e.target.value)}
          >
        ...
        <FormControl sx={{ minWidth: 120 } as any}>
          ...
          <Select
            value={sortBy}
            label="Sort By"
            onChange={(e: any) => (e: any) => (e: any) => setSortBy(e.target.value)}
          >
      ...
      {/* Customers Grid */}
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: 2 } as any}>
        {filteredCustomers.map((customer) => (
          <Card key={customer.id} sx={{ height: 'fit-content' } as any}>
            <CardContent>
              <Stack spacing={2}>
                {/* Customer Header */}
                ...
                {/* Customer Stats */}
                <Stack direction="row" justifyContent="space-between" sx={{ pt: 1, borderTop: 1, borderColor: 'divider' } as any}>
                  <Box>
      ...
      {filteredCustomers.length === 0 && (
        <Box sx={{ textAlign: 'center', py: 4 } as any}>
          <PersonIcon sx={{ fontSize: 64, color: 'text.secondary', mb: 2 } as any} />
