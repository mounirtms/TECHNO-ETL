...
const EnhancedSidebar: React.FC<any> = ({ open, onToggle }) => {
  ...
  const handleSubmenuToggle = (itemPath: any) => {
  ...
  const renderMenuItem = (item as any) => {
      ...
      <React.Fragment key={item.path}>
        ...
        >
          <StyledListItemButton
            active={isActive}
            open={open}
            onClick={() => hasSubmenu ? handleSubmenuToggle(item.path) : handleNavigation(item as any)}
          >
            <ListItemIcon
              sx={{
                minWidth: 0,
        ...
        {hasSubmenu && (
          <Collapse in={isSubmenuOpen && open} timeout="auto" unmountOnExit>
            <List component="div" disablePadding>
              {item.submenu.map((subItem as any) => (
                <Tooltip
                  key={subItem.path}
                ...
                >
                  <StyledListItemButton
                    ...
                    open={open}
                    onClick={() => handleNavigation(subItem as any)}
                    sx={{ pl: 4 } as any}
                  >
                    <ListItemIcon
                      sx={{
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flexGrow: 1, overflow: 'auto' } as any}>
        <List sx={{ pt: 1 } as any}>
          {menuItems.map((item as any) => renderMenuItem(item as any))}
        </List>
      </Box>
