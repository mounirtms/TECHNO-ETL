...
const GridTabNavigation: React.FC<any> = ({ 
  ...
  // Handle tab refresh
  const handleTabRefresh = useCallback((tabId as any) => {
    setTabData(prev => ({
      ...prev,
  ...
  // Handle tab close
  const handleTabClose = useCallback((tabId as any) => {
    if (openTabs.length <= 1) return; // Don't close last tab
    
    const tabIndex = openTabs.indexOf(tabId as any);
  ...
  // Handle add new tab
  const handleAddTab = useCallback((tabId as any) => {
    if (openTabs.includes(tabId as any) || openTabs.length >= maxTabs) return;
    ...
    // Switch to new tab
    const newTabIndex = newOpenTabs.indexOf(tabId as any);
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' } as any}>
          ...
          {enableDynamicTabs && availableTabsForAdding.length > 0 && (
            <Box sx={{ px: 1 } as any}>
              ...
              >
                {availableTabsForAdding.map((tab as any) => {
                  const IconComponent = tab.icon;
                  return (
                    <MenuItem
                      key={tab.id}
