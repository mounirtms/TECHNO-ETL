...
const GridCardView: React.FC<any> = ({ data = [], type = 'product' }) => {
    ...
    const renderProductCard = (item as any) => (
        <Grid {...{item: true}} xs={12} sm={6} md={4} lg={3} key={item.id || item.sku}>
            <StyledCard>
                <Box sx={{ position: 'relative' } as any}>
                    <StyledCardMedia
                    ...
                    <Stack spacing={1}>
                        <InfoRow>
                            ...
                            <Typography variant="body2">
                                ...
                                {item.special_price && (
                                    <Typography component="span" color="error.main" sx={{ ml: 1 } as any}>
    ...
    const renderOrderCard = (item: any) => (
        <Grid {...{item: true}} xs={12} sm={6} md={4} lg={3} key={item.entity_id || item.increment_id}>
            <StyledCard>
                <CardContent>
    ...
    const renderCustomerCard = (item: any) => (
        <Grid {...{item: true}} xs={12} sm={6} md={4} lg={3} key={item.id || item.email}>
            <StyledCard>
                <CardContent>
                    <Stack direction="row" justifyContent="space-between" alignItems="center">
                        <Typography variant="h6" noWrap>
    ...
    const renderGenericCard = (item: any, index: any) => (
        <Grid {...{item: true}} xs={12} sm={6} md={4} lg={3} key={item.id || index}>
            <StyledCard>
                <CardContent>
    ...
    return (
        <Box sx={{ p: 2 } as any}>
                ...
                {data.map((item, index) => {
                    switch (type) {
                        case 'ProductsGrid':
                            return renderProductCard(item as any);
                        case 'OrdersGrid':
                            return renderOrderCard(item as any);
                        case 'CustomersGrid':
                            return renderCustomerCard(item as any);
                        default:
