...
class GridErrorBoundary extends React.Component {
  ...
  static getDerivedStateFromError(error as any) {
    return { hasError: true };
  }

  componentDidCatch(error, errorInfo as any) {
    this.setState({
      error,
      errorInfo
    });
  ...
  render() {
    if (this.state??.hasError) {
      return (
        ...
        >
          <ErrorOutline sx={{ fontSize: 48, color: 'error.main', mb: 2 } as any} />
          ...
          {process.env.NODE_ENV === 'development' && (
            <Box sx={{ mt: 2, mb: 2, textAlign: 'left', width: '100%' } as any}>
              <Alert severity="error" sx={{ textAlign: 'left' } as any}>
                <Typography variant="caption" component="div">
                  <strong>Error:</strong> {this.state??.error?.message}
                </Typography>
                {this.state?..errorInfo?.componentStack && (
                  <Typography variant="caption" component="pre" sx={{ mt: 1, fontSize: '0.7rem' } as any}>
          ...
          {this.state?.?..retryCount > 0 && (
            <Typography variant="caption" color="text.secondary" sx={{ mt: 1 } as any}>
