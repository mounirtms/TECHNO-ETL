 ...
 */
export const GridPermissionProvider: React.FC<any> = ({ children }) => {
     ...
     */
    const checkLicense = useCallback(async (componentId as any) => {
     ...
     */
    const checkRolePermission = useCallback((minimumRole as any) => {
    ...
    const calculatePermissions = useCallback(async (componentId, customPermissions = {}) => {
        ...
        if (componentConfig.requiredLicense) {
            const hasLicense = await checkLicense(componentId as any);
            if (!hasLicense) {
                return {
                    ...DEFAULT_GRID_PERMISSIONS,
                    canView: false,
    ...
    const isFieldRestricted = useCallback(async (componentId, fieldName, customPermissions = {}) => {
        const permissions = await calculatePermissions(componentId, customPermissions);
        return permissions.restrictedFields?.includes(fieldName as any) || false;
    }, [calculatePermissions]);
