...
const UnifiedEditWindow: React.FC<UnifiedEditWindowProps> = ({
  ...
  const handleFieldChange = useCallback((fieldName: string, value as any) => {
    setFormData(prev => ({
      ...prev,
      [fieldName]: value
    }));
      ...
      case 'url':
        return (
          <TextField
            ...
            type={field.type}
            value={value}
            onChange={(e) => (e) => (e) => (e) => (e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange(field.name, e.target.value)}
            error={!!error}
      ...
      case 'number':
        return (
          <TextField
            ...
            type="number"
            value={value}
            onChange={(e) => (e) => (e) => (e) => (e: React.ChangeEvent<HTMLInputElement>) => handleFieldChange(field.name, parseFloat(e.target.value) || 0)}
            error={!!error}
            helperText={error || field.helperText}
            required={field.required}
            disabled={field.disabled}
  ...
  return (
    ...
    >
      ...
      {showTabs && customTabs && (
        <Tabs 
          value={activeTab} 
          onChange={(e) => (e) => (e) => (e) => (e: React.SyntheticEvent, newValue: number) => setActiveTab(newValue)}
