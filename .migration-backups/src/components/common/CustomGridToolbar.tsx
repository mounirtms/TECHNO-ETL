...
const CustomGridToolbar = ({
  ...
  const handleCustomFilterChange = async (event: any) => {
    try {
  ...
  // --- Toolbar Layout ---
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', p: 1 } as any}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 } as any}>
        ...
        {succursaleOptions?.length > 0 && (
          <FormControl variant="outlined" size="small" sx={{ minWidth: 120 } as any}>
            <InputLabel>Succursale</InputLabel>
            <Select value={currentSuccursale} onChange={(e: any) => (e: any) => (e: any) => (e: any) => (e: any) => (e: any) => (e: any) => (e: any) => onSuccursaleChange?.(e.target?.value)} label="Succursale" sx={toolbarButtonStyle}>
        ...
        {sourceOptions?.length > 0 && (
          <FormControl variant="outlined" size="small" sx={{ minWidth: 120 } as any}>
            ...
            <Select value={currentSource} onChange={(e: any) => (e: any) => (e: any) => (e: any) => (e: any) => (e: any) => (e: any) => (e: any) => onSourceChange?.(e.target?.value)} label="Source" sx={toolbarButtonStyle}>
        ...
        {typeof showChangedOnly !== 'undefined' && typeof setShowChangedOnly === 'function' && canSyncAll && (
          <TooltipWrapper title="Show only products with inventory changes">
            <Box sx={{ display: 'flex', alignItems: 'center', pl: 1 } as any}>
              <Checkbox
                checked={showChangedOnly}
                onChange={(e: any) => (e: any) => (e: any) => (e: any) => (e: any) => (e: any) => (e: any) => e => setShowChangedOnly(e.target.checked)}
                color="primary"
                size="small"
                inputProps={{ 'aria-label': 'Show only products with inventory changes' }}
              />
              <Typography variant="body2" sx={{ whiteSpace: 'nowrap' } as any}>
        ...
        {customFilters?.length > 0 && (
          <FormControl variant="outlined" size="small" sx={{ minWidth: 120 } as any}>
            ...
            <Select value={currentCustomFilter} onChange={(e: any) => (e: any) => (e: any) => (e: any) => (e: any) => (e: any) => (e: any) => handleCustomFilterChange} label="Custom Filter" sx={toolbarButtonStyle}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 } as any}>
        ...
        {canInfo && (
          <TooltipWrapper title="Product Info">
            <Button variant="outlined" size="small" onClick={onInfo} sx={{ ...toolbarButtonStyle, minWidth: 40, px: 1.5, ml: 1 } as any}>
              Info
            </Button>
          </TooltipWrapper>
        )}
