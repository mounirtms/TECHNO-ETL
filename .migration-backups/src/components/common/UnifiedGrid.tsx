...
const UnifiedGrid = forwardRef<any, UnifiedGridProps>(({
  ...
  showStatsCards = true,
  showCardView = true,
  defaultViewMode = 'grid' as 'grid' | 'card',
  gridCards = [],
  totalCount = 0,
  defaultPageSize = 25,
  paginationMode = "client" as 'client' | 'server',
  ...
  const handleSelectionChange = useCallback((newSelection as any) => {
    setSelectedRows(newSelection);
    onSelectionChange?.(newSelection);
  }, [setSelectedRows, onSelectionChange]);

  // Context menu handlers
  const handleContextMenu = useCallback((event: React.MouseEvent, row as any) => {
  ...
  // View mode toggle
  const handleViewModeChange = useCallback((_event: any, newMode: 'grid' | 'card') => {
    if (newMode !== null) {
      setViewMode(newMode);
    }
  }, []);
  ...
  return (
    <>
      ...
      {/* Main Content Area */}
      {loading && (
        <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 1 } as any}>
        ...
        <Box
          ...
          {viewMode === 'card' && showCardView ? (
            ...
            <DataGrid
              ...
              onPaginationModelChange={useCallback((model as any) => {
              ...
              sortModel={sortModel}
              onSortModelChange={useCallback((model as any) => {
                setSortModel?.(model);
                onSortChange?.(model);
              ...
              onFilterModelChange={useCallback((model as any) => {
...
const MemoizedUnifiedGrid = memo(UnifiedGrid, (prevProps: any, nextProps as any) => {
