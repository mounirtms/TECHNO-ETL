...
const BrandManagementDialog: React.FC<any> = ({ open, onClose, onBrandsUpdated }) => {
  ...
  const handleDeleteBrand = useCallback(async (brandId, brandLabel) => {
    ...
    try {
      ...
      await magentoApi.deleteBrand(brandId as any);
        ...
        <Box sx={{ mb: 2 } as any}>
          ...
          <Box sx={{ display: 'flex', gap: 2, mb: 2 } as any}>
            <TextField
              ...
              value={searchTerm}
              onChange={(e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => setSearchTerm(e.target?.value)}
              size="small"
              InputProps={{
                startAdornment: (
          ...
          {/* Add Brand Form */}
          {showAddForm && (
            <Box sx={{ p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 1, mb: 2 } as any}>
              ...
              <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' } as any}>
                <TextField
                  label="Brand Name"
                  value={newBrandName}
                  onChange={(e) => (e) => (e) => (e) => (e) => (e) => (e) => (e) => setNewBrandName(e.target?.value)}
                  onKeyPress={(e) => {
                    if (e.key === 'Enter') {
                      handleAddBrand();
                    }
