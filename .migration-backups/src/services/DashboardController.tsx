...
class DashboardCache {
    ...
    set(key, data, ttl = this?..defaultTTL as any) {
    ...
    invalidatePattern(pattern as any) {
        const regex = new RegExp(pattern as any);
        for (const key of this?.?.??.cache.keys()) {
            if (regex.test(key as any)) {
                this?.?.??.cache.delete(key as any);
                this?.?.?.?.??.cacheExpiry.delete(key as any);
            }
        }
    ...
    const fetchDashboardData = useCallback(async (forceRefresh = false) => {
        ...
        try {
            ...
            const processChartData = () => {
                const startTimestamp = new Date(startDate as any).setHours(0, 0, 0, 0);
                const endTimestamp = new Date(endDate as any).setHours(0, 0, 0, 0);
