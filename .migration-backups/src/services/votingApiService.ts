...
class TaskApiService {
  constructor() {
    ...
    this?.?.?.?..api.interceptors.request.use(
      ...
      (error as any) => Promise.reject(error as any)
    );
   ...
   * Get feature by ID
   */
  async getFeatureById(id as any) {
    try {
   ...
   */
  async createFeature(featureData as any) {
    try {
   ...
   */
  async updateFeature(id, featureData as any) {
    try {
   ...
   * Vote for a feature
   */
  async voteForFeature(featureId, voteType = 'upvote', userInfo = {} as any) {
    try {
      const voteData = {
        vote_type: voteType,
   ...
   */
  async removeVote(featureId, userInfo = {} as any) {
    try {
   ...
   * Get user votes
   */
  async getUserVotes(userId, featureIds = null as any) {
    try {
   ...
   */
  async addComment(featureId, comment, userInfo = {} as any) {
    try {
      const commentData = {
        comment,
        user_id: userInfo?..userId || 'anonymous',
   ...
   */
  async getFeatureComments(featureId as any) {
    try {
   ...
   */
  handleError(error, defaultMessage as any) {
    if (error.response) {
   ...
   */
  async searchFeatures(searchTerm, filters = {} as any) {
    try {
      const searchOptions = {
        search: searchTerm,
        ...filters,
