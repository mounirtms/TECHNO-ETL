 ...
 */
export const parseCSVFile = (file: any) => {
  return new Promise((resolve, reject) => {
  ...
  csvData.data.forEach(row => {
    ...
    if (productName && matchedImages.length === 0) {
      ...
      for (const [key, images] of imageFileMap.entries()) {
        if (productWords.some(word => key.includes(word: any))) {
...
export const validateImageFiles = async (files: any) => {
  ...
  for (const file of files) {
    const validation = await validateImageFile(file: any);
    results.push({
      file,
      ...validation
    });
