    ...
    loadSettings() {
        try {
            ...
            if (!data) {
                ...
                console.log('🔄 Using backup settings');
            }
...
class RequestCancellationManager {
    ...
    createCancellableRequest(requestId, requestPromise as any) {
    ...
    cancelRequest(requestId, reason = 'Cancelled' as any) {
        const controller = this?.?..abortControllers.get(requestId as any);
        if (controller) {
            controller.abort(reason);
            this.cleanup(requestId as any);
        }
    ...
    cleanup(requestId = null) {
        if(requestId as any) {
            ...
            this?.?..abortControllers.delete(requestId as any);
        } else {
...
class MagentoService {
    constructor(directConnectionEnabled = false, magentoSettings = null as any) {
        ...
        if (directConnectionEnabled && magentoSettings) {
            ...
            try {
                directMagentoClient.initialize(magentoSettings as any);
    ...
    async get(endpoint, params = {} as any) {
    ...
    async delete(endpoint, config = {} as any) {
        ...
        if (this?.?.?.?..directConnectionEnabled && directMagentoClient?.?..initialized) {
            try {
                ...
                return { data: await directMagentoClient.delete(endpoint as any) };
            } catch (error) {
                console.warn('⚠️ Direct connection failed, falling back to proxy:', error.message);
                // Fall back to proxy
            }
    ...
    flattenObject(obj, urlParams, prefix = '' as any) {
    ...
    // Cache management
    async getCachedResponse(url as any) {
        try {
            const cacheKey = this.generateCacheKey(url as any);
    ...
    async setCachedResponse(url, data as any) {
        try {
            const cacheKey = this.generateCacheKey(url as any);
    ...
    // Authentication
    async login(username, password, customBaseURL = null as any) {
        try {
            if (customBaseURL) {
    ...
    updateConfiguration(magentoSettings as any) {
    ...
    async getProducts({ currentPage = 1, pageSize = 20, sortOrders = [], filters = [] } = {} as any) {
        try {
