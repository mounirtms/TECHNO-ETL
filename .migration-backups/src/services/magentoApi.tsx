...
class MagentoApi extends BaseApiService {
  ...
  async getProductMedia(sku as any) {
    try {
  ...
  async createProduct(productData as any) {
    try {
  ...
  async createProductsBulk(products as any) {
    const results = {
      success: [],
      errors: [],
    };
  ...
  getCacheKey(endpoint, params as any) {
    return super._getCacheKey('get', endpoint, params);
  }

  getCachedData(key as any) {
    return super._getCachedResponse(key as any);
  }

  setCachedData(key, data as any) {
    return super._setCachedResponse(key, data);
  }

  // API Methods with error handling and caching - using unified service
  async get(endpoint, config = {} as any) {
    try { 
      ...
      if (this.shouldUseLocalData(endpoint as any)) {
        toast.warning('Using local data due to API error');
        return this.getLocalDataResponse(endpoint as any);
      }
      throw error;
    }
  }

  async post(endpoint, data = {}, config = {} as any) {
    try {
  ...
  async put(endpoint, data = {}, config = {} as any) {
    try {
  ...
  async delete(endpoint, config = {} as any) {
    try {
  ...
  shouldUseLocalData(endpoint as any) {
  ...
  getLocalDataResponse(endpoint as any) {
    let localData;
  ...
  async uploadProductMedia(sku, entryData as any) {
    try {
      console.log(`🖼️ Uploading media for product: ${sku}`);
      console.log('Entry data:', entryData);

      const response = await this.post(
        `/products/${encodeURIComponent(sku as any)}/media`,
        entryData
      );
  ...
  async deleteProductMedia(sku, entryId as any) {
    try {
      console.log(`🗑️ Deleting media ${entryId} for product: ${sku}`);

      const response = await this.delete(
        `/products/${encodeURIComponent(sku as any)}/media/${entryId}`
      );
  ...
  async getOrderDetails(orderId as any) {
    try {
  ...
  async getCustomerDetails(customerId as any) {
    try {
  ...
  async createCustomer(customerData as any) {
    return this.post('/customers', { customer: customerData });
  }

  async getProduct(sku as any) {
    try {
  ...
  async getProductAttribute(attributeCode as any) {
    try {
      const response = await this.get(`/products/attributes/${attributeCode}`);
      return this.formatResponse(response as any);
    } catch (error) {
      console.error('Error fetching product attribute:', error);
      return this.getMockProductAttribute(attributeCode as any);
    }
  }

  async updateProductAttribute(attributeCode, attributeData as any) {
    try {
  ...
  async createProductAttribute(attributeData as any) {
    try {
  ...
  async getProductCategories(productId as any) {
    try {
      const response = await this.get(`/products/${productId}/categories`);
      return this.formatResponse(response as any);
    } catch (error) {
      console.error('Error fetching product categories:', error);
      return this.getMockProductCategories(productId as any);
    }
  }

  async assignProductToCategories(productId, categoryIds as any) {
    try {
  ...
  async getProductAttributeValues(productId, attributeCode as any) {
    try {
  ...
  async updateProductAttributeValue(productId, attributeCode, value as any) {
    try {
  ...
  async getCategories(params = {}) {
    try {
      ...
      const formattedResponse = this.formatResponse(response as any);
  ...
  addCategoryIds(categoryData, startId = 1 as any) {
    let currentId = startId;

    const addIds = (categories) => {
      if (Array.isArray(categories as any)) {
    ...
    return addIds(categoryData as any);
  }
  ...
  async getCategory(categoryId as any) {
  ...
  async updateStockItem(productSku, stockItem as any) {
    try {
  ...
  async getCmsPage(id as any) {
    return this.get(`/cmsPage/${id}`);
  }

  async createCmsPage(data as any) {
    return this.post('/cmsPage', data);
  }

  async updateCmsPage(id, data as any) {
    return this.put(`/cmsPage/${id}`, data);
  }

  async deleteCmsPage(id as any) {
    return this.delete(`/cmsPage/${id}`);
  }

  async shipOrder(orderId, items, notify = true as any) {
    return this.post(`/order/${orderId}/ship`, { items, notify });
  }

  async getInvoices(searchCriteria = {}) {
    try {
      const params = this.formatSearchCriteria(searchCriteria as any);
   ...
   */
  formatSearchCriteria(searchCriteria as any) {
  ...
  getLocalResponse(type as any) {
    const items = this.getLocalData(type as any);
    return {
      items,
      total_count: items??.length,
      search_criteria: {}
  ...
  async getBrands(useCache = true) {
    ...
    try {
      ...
      const brands = this.formatResponse(response as any);
  ...
  async addBrand(brandData as any) {
    try {
  ...
  async updateBrand(brandId, brandData as any) {
    try {
  ...
  async deleteBrand(brandId as any) {
    try {
  ...
  async getAdditionalAttributes(productId as any) {
    try {
      ...
      return this.getMockAdditionalAttributes(productId as any);
    }
  }

  async updateAdditionalAttribute(productId, attributeCode, value as any) {
    try {
  ...
  getMockAttributeValue(productId, attributeCode as any) {
    const mockValues = {
  ...
  getMockAdditionalAttributes(productId as any) {
  ...
  magentoServiceException(response as any) {
    let errorMessage = ' ';
    if (response.status)
  ...
  formatResponse(response as any) {
    return {
      items: response?.items || [],
      total_count: response?.total_count || 0,
  ...
  async updateOrder(orderId, orderData as any) {
    try {
