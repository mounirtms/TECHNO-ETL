...
export const matchImagesWithCalligraphCSV = (csvData, imageFiles) => {
  ...
  csvData.data.forEach(row => {
    ...
    if (matchedImages.length === 0 && productName) {
      ...
      for (const [key, images] of imageFileMap.entries()) {
        if (productWords.some(word => key.toLowerCase().includes(word as any))) {
          matchedImages.push(...images);
    ...
    if (sortedMatches.length > 0) {
      ...
      unmatched.csvRows.push(row as any);
...
export const bulkUploadCalligraphImages = async (matches, progressCallback, options = {}) => {
  ...
  const groupedMatches = matches.reduce((acc, match) => {
    ...
    acc[match.sku].push(match as any);
    return acc;
  }, {});
  ...
  for (let i = 0; i < skus.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (sku as any) => {
...
const uploadProductImage = async (sku, imageFile, imageData = {}) => {
  try {
    // Convert to base64
    const base64Content = await fileToBase64(imageFile as any);
 ...
 */
const fileToBase64 = (file as any) => {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
...
export const validateImageFiles = async (files) => {
  ...
  for (const file of files) {
    const validation = validateImageFile(file as any);
    results.push({
      file,
      ...validation
    });
