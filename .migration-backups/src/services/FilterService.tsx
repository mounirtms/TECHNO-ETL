...
class FilterService {
  ...
  setCacheData(key, data as any) {
    this?.?..cache.set(key, data);
    this?.?.?.??.cacheExpiry.set(key, Date.now() + this?..CACHE_DURATION);
    console.log(`📦 Cached data for key: ${key}`);
  }

  getCacheData(key as any) {
    const expiry = this?.?.?.??.cacheExpiry.get(key as any);
    ...
    this?.?.?.??.cacheExpiry.delete(key as any);
    return null;
  }
  ...
  async getCategories(useCache = true) {
    ...
    try {
      ...
      const processedCategories = this.flattenCategories(categories as any);
  ...
  async getAttributeOptions(attributeCode, useCache = true as any) {
    ...
    try {
      const response = await magentoApi.getProductAttribute(attributeCode as any);
  ...
  buildMagentoSearchCriteria(filters as any) {
  ...
  performSearch(searchTerm, callback as any) {
    console.log('🔍 Performing search:', searchTerm);
    callback(searchTerm as any);
  }

  performFilter(filters, callback as any) {
    console.log('🔄 Performing filter:', filters);
    callback(filters as any);
  }

  // ===== CLIENT-SIDE FILTERING =====

  applyClientFilters(products, filters as any) {
  ...
  flattenCategories(categories, level = 0, result = [] as any) {
