...
class CegidApi {
    ...
    createSoapEnvelope(body, database as any) {
    ...
    createHelloWorldBody(database as any) {
        return `
            <ret:HelloWorld>
    ...
    parseSoapResponse(xmlString as any) {
        try {
    ...
    createBasicAuth(username, password as any) {
        try {
    ...
    async handleCegidConnect(
        ...
        password = this?.?.?..credentials.password, database = this?.?.?..credentials.database
     as any) {
        try {
            // Cancel any ongoing requests
            ...
            const soapRequest = this.createSoapEnvelope(
                this.createHelloWorldBody(database as any),
                database
            );
    ...
    async fetchData(action, body, options = {} as any) {
        try {
    ...
    createSearchProductSoapEnvelope(action, parameter, request as any) {
        return `<?xml version="1.0" encoding="utf-8"?>
        <soapenv:Envelope 
    ...
    parseProductSearchResponse(xmlResponse as any) {
        try {
