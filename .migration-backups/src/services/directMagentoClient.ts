...
class DirectMagentoClient {
     ...
     */
    initialize(magentoSettings as any) {
     ...
     */
    async post(endpoint, data = {} as any) {
        this.checkInitialization();
     ...
     * @param {Object} data - Request body
     */
    async put(endpoint, data = {} as any) {
        this.checkInitialization();
     ...
     * @param {string} endpoint - API endpoint
     */
    async delete(endpoint as any) {
        this.checkInitialization();
        
        try {
            const response = await this?.?.?.?.instance.delete(endpoint as any);
            return response.data;
        } catch (error as any) {
    ...
    async getProduct(sku as any) {
        return await this.get(`/products/${encodeURIComponent(sku as any)}`);
    }
     ...
     */
    async updateProduct(sku, productData as any) {
        return await this.put(`/products/${encodeURIComponent(sku as any)}`, { product: productData });
    }
     ...
     * @param {string} sku - Product SKU
     */
    async deleteProduct(sku as any) {
        return await this.delete(`/products/${encodeURIComponent(sku as any)}`);
    }
     ...
     */
    buildSearchCriteriaParams(criteria as any) {
        const params = {};
     ...
     */
    async login(username, password as any) {
        if (!this?.?.??.baseURL) {
