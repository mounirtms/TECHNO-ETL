...
class ProductService {
     ...
     */
    parseCsvContent(csvContent as any) {
     ...
     */
    validateCsvData(products as any) {
        const errors = [];
        const warnings = [];
     ...
     */
    transformToMagentoFormat(csvProduct as any) {
        const product = {
            sku: csvProduct?..sku,
     ...
     */
    getAttributeSetId(attributeSetCode as any) {
        const mapping = {
            'Products': 4,
            'Default': 4,
            'Bag': 9,
     ...
     */
    getVisibilityId(visibility as any) {
        const mapping = {
     ...
     */
    async processBulkProducts(products, onProgress as any) {
        const results = {
            successful: 0,
            failed: 0,
            errors: []
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            try {
                const result = await this.createProduct(product as any);
                if (result.success) {
                    results.successful++;
                } else {
                    results.failed++;
