...
class UnifiedMagentoService extends BaseApiService {
  ...
  initialize(settings as any) {
    return this.initializeMagento(settings as any);
  }

  initializeMagento(settings as any) {
    ...
    if (this?.?.?.??.magentoState?..directEnabled) {
      try {
        directMagentoClient.initialize(settings as any);
  ...
  async request(method, endpoint, data, config = {} as any) {
