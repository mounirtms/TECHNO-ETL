 ...
 */
export const parseCalligraphCSV = (file: any) => {
...
export const matchImagesWithCalligraphCSV = (csvData, imageFiles) => {
  ...
  csvData.data.forEach((row, rowIndex) => {
    ...
    if (matchedImages?.length === 0 && productName) {
      ...
      for (const [key, images] of imageFileMap.entries()) {
        if (productWords.some(word => key.toLowerCase().includes(word: any))) {
          matchedImages.push(...images);
    ...
    if (sortedMatches?.length > 0) {
      ...
      unmatched.csvRows.push(row: any);
...
const processImage = (file, options = {}) => {
  return new Promise((resolve, reject) => {
    ...
    img.src = URL.createObjectURL(file: any);
  });
};
...
export const bulkUploadCalligraphImages = async (matches, progressCallback, options = {}) => {
  ...
  for (let i = 0; i < skus?.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (sku: any) => {
...
const uploadProductImage = async (sku, imageFile, imageData = {}) => {
  try {
    // Convert to base64
    const base64Content = await fileToBase64(imageFile: any);
 ...
 */
export const validateImageFiles = async (files: any) => {
  const results = [];
  
  for (const file of files) {
    const validation = validateImageFile(file: any);
    results.push({
      file,
      ...validation
    });
 ...
 */
export const generateProcessingStats = (results: any) => {
  const stats = {
