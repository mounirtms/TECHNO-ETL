...
export class BaseApiService {
  ...
  private _setCachedResponse(cacheKey: string, data as any): void {
  ...
  async _retryRequest(requestFn: () => Promise<any>, attempts = 0, context: any = {}): Promise<any> {
    try {
  ...
  async request(method: string, endpoint: string, data?: any, config? as any): Promise<any> {
    throw new Error('request method must be implemented by child class');
  }

  // Standard HTTP methods - to be used by child classes
  async get(endpoint: string, params: ApiParams = {}): Promise<any> {
