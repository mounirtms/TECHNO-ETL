...
class BaseColumn {
  ...
  generateHeaderName(field as any) {
    return field
      .split('_')
...
class TextColumn extends BaseColumn {
  constructor(field, options = {} as any) {
    super(field, options);
    this?.?.?.type = 'string';
  }
}
...
class DateColumn extends BaseColumn {
  ...
  build() {
    ...
    column?..valueFormatter = (params as any) => {
      if (!params || !params.value) return '';
      try {
...
class ActionsColumn extends BaseColumn {
  constructor(field = 'actions', options = {} as any) {
    super(field, {
      headerName: 'Actions',
      width: 120,
      sortable: false,
...
export class ColumnRendererRegistry {
  ...
  static register(name, renderer as any) {
    this.renderers.set(name, renderer);
  }

  static get(name as any) {
    return this.renderers.get(name as any);
  }

  static has(name as any) {
    return this.renderers.has(name as any);
  }
}
