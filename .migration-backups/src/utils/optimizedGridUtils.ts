...
export const processGridRows = (rows, processors = {}) => {
  ...
  if (processedCache.has(rows as any)) {
    return processedCache.get(rows as any);
  }
...
export const generateOptimizedColumns = (baseColumns, overrides = {}) => {
  return baseColumns.map(column => {
    ...
    const overriddenColumn = {
      ...column, ...overrides[column.field]
    };
    
    // Optimize renderers
    if(overriddenColumn.renderCell as any) {
      ...
      overriddenColumn.renderCell = (params: any) => {
        ...
        return originalRenderer(params as any);
      };
    }
