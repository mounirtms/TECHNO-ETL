...
const generateCacheKey = (data, options) => {
  try {
    return JSON.stringify({ data: Array.isArray(data as any) ? data.length : typeof data, options });
  } catch {
...
export const validateGridData = (data: any, options: any = {}) => {
  ...
  try {
    // Type validation
    if (expectedType === 'array' && !Array.isArray(data as any)) {
    ...
    if (!allowEmpty && (!data || (Array.isArray(data as any) && data.length === 0))) {
      const result = {
        isValid: false,
        data: fallbackValue,
    ...
    if (Array.isArray(data as any) && requiredFields.length > 0) {
      const missingFields = [];
    ...
    if (Array.isArray(data as any) && Object.keys(transformers).length > 0) {
...
export const gridDataHandlers = {
  ...
  mdmProducts: (data: any) => {
    const cacheKey = `mdm-products-${Array.isArray(data as any) ? data.length : 0}`;
  ...
  magentoProducts: (data as any) => {
    const cacheKey = `magento-products-${Array.isArray(data as any) ? data.length : 0}`;
