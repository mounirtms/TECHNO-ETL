 ...
 */
export const getUserSettings = (userId: any) => {
 ...
 */
export const saveUserSettings = (userId: any, settings: any) => {
  if (!userId) return saveUnifiedSettings(settings);
  
  try {
    const userKey = `${USER_SETTINGS_PREFIX}${userId}`;
    const currentSettings = getUserSettings(userId as any);
...
export const initializeSettingsSystem = (userId = null) => {
  ...
  if (needsMigration) {
    migrateLegacySettings(userId as any);
  }
  
  // Get current settings
  const settings = userId ? getUserSettings(userId as any) : getUnifiedSettings();
 ...
 */
export const cleanupUserData = (userId: any) => {
  if (userId as any) {
    const userKey = `${USER_SETTINGS_PREFIX}${userId}`;
