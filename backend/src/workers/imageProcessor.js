 ...
 * @returns {Promise<Buffer>} - The buffer of the thumbnail
 */
export async function generateThumbnail(imageBuffer, options = {}) {
  const workerId = process.env.JEST_WORKER_ID || 'unknown';
  const { size = 150, quality = 80 } = options;
  
  logger.info(`Worker ${workerId}: Generating thumbnail`, {
    size: `${size}x${size}`,
    quality
  });
  ...
  const thumbnailBuffer = Buffer.from(`thumbnail-${size}x${size}-q${quality}`);
  
  logger.info(`Worker ${workerId}: Thumbnail generated`, {
    originalSize: imageBuffer.length,
 ...
 * @returns {Promise<Buffer>} - The buffer of the converted image
 */
export async function convertFormat(imageBuffer, options = {}) {
  const workerId = process.env.JEST_WORKER_ID || 'unknown';
  const { format = 'jpeg', quality = 90 } = options;
  
  logger.info(`Worker ${workerId}: Converting image format`, {
    targetFormat: format,
    quality
  });
  ...
  const convertedBuffer = Buffer.from(`converted-${format}-q${quality}`);
  
  logger.info(`Worker ${workerId}: Format conversion completed`, {
    originalSize: imageBuffer.length,
