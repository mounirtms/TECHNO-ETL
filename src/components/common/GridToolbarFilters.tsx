    ...
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
      ...
      {customFilters.length > 0 && (
        <FormControl size="small" sx={{ display: "flex", minWidth: 120 }}></
          ...
          <Select value={currentCustomFilter || 'all'}
      ...
      {succursaleOptions.length > 0 && (
        <FormControl size="small" sx={{ display: "flex", minWidth: 120 }}></
          ...
          <Select value={currentSuccursale || 'all'}
      ...
      {sourceOptions.length > 0 && (
        <FormControl size="small" sx={{ display: "flex", minWidth: 120 }}></
          ...
          <Select value={currentSource || 'all'}
      ...
      {typeof showChangedOnly !== 'undefined' && setShowChangedOnly && (<FormControlLabel control
      ...
      {(currentCustomFilter && currentCustomFilter !== 'all') && (
        <Chip label={`${translate('filter', 'Filter')}: ${currentCustomFilter}`}
          size="small"
      ...
      {(currentSuccursale && currentSuccursale !== 'all') && (
        <Chip label={`${translate('branch', 'Branch')}: ${currentSuccursale}`}
          size="small"
      ...
      {(currentSource && currentSource !== 'all') && (
        <Chip label={`${translate('source', 'Source')}: ${currentSource}`}
          size="small"
      ...
      {showChangedOnly && (
        <Chip label={translate('changedOnly', 'Changed Only')}
          size="small"
