        ...
        <Box sx={{ display: "flex", p: 2 }}>
          <Box sx={{
            ...
            <FormControl size="small"
              ...
              >
                {sourceFilterOptions.map((option: any) => (
                  <MenuItem key={option?.value}
                    value={option?.value}
                    sx={{
                      gap: 1,
            ...
            <>
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                  {succursaleFilter !== '16' && (
                    <Chip label={`Branch: ${succursaleOptions.find(o => o?.value ===succursaleFilter)?.label || succursaleFilter}`}
                      size="small"
                      onDelete={() => onSuccursaleChange('16')}
                      color
                  )}
                  {sourceFilter !== 'all' && (
                    <Chip label={`Source: ${sourceFilterOptions.find(o => o?.value ===sourceFilter)?.label || sourceFilter}`}
                      size="small"
